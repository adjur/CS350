...
class TemperatureMachine(StateMachine):
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            lcd_line_1 = current_time.strftime("%m/%d %I:%M %p")[:16] + "\n"
    
            ## Setup Display Line 2
            if(altCounter < 6):
                lcd_line_2 = f"Temp: {self.getFahrenheit():.1f}F".ljust(16)[:16]
    
                altCounter = altCounter + 1
            else:
                lcd_line_2 = f"{self.current_state.id.upper()} SP:{self.setPoint}F".ljust(16)[:16]
